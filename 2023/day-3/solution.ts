...
async function loadData() {
    ...
    const file = Bun.file("../../inputs/2023/day-3/input.txt");
...
function partOne(data: string) {
    ...
    for (let i = 0; i < numberIndices.length; i++) {
        ...
        if (numberIndices[i + 1] !== undefined && numberIndices[i + 1][0] == x + 1) {
            ...
            numberIndices2.push([[x, numberIndices[i][1]]]);
        }
    }

    // for each number check if in the symbolIndices array, theres is symbol +1x -1x, +1y -1y, or +1y+1x-1x -1y+1x-1x
    for (let ni = 0; ni < numberIndices2.length; ni++) {
        // for each number
        for (let di = 0; di < numberIndices2[ni].length; di++) {
            // for each digit
            let dx = numberIndices2[ni][di][0];
            let dy = numberIndices2[ni][di][1];

            for (let si = 0; si < symbolIndices.length; si++) {
                // check each symbol
                let sx = symbolIndices[si][0];
                let sy = symbolIndices[si][1];

                if ((dx + 1 == sx && dy == sy) || (dx - 1 == sx && dy == sy)) {
                    // look right, look left on the same y
                    partsIndices.push(numberIndices2[ni]);
                } else if ((dy + 1 == sy && dx == sx) || (dy - 1 == sy && dx == sx)) {
                    // look up and down on the same x
                    partsIndices.push(numberIndices2[ni]);
                } else if (
                    (dx + 1 == sx && dy + 1 == sy) ||
                    (dx - 1 == sx && dy + 1 == sy) ||
                    ...
                    // move up or down, look left or right
                    partsIndices.push(numberIndices2[ni]);
                }
            }
        }
...
function partTwo(data: string) {
    ...
    const filteredData = Array.from(indexMap.values()).filter((numberArrays) => numberArrays.length > 1);
