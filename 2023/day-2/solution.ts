...
async function loadData() {
    ...
    const file = Bun.file("../../inputs/2023/day-2/input.txt");
...
function partOne(data: string) {
    ...
    data.split("\n").forEach((line) => {
        ...
        gameSets.forEach((item) => {
            // for each set (ex: 1 red, 3 blue, 4 green)
            ...
            cubes.forEach((cube) => {
                ...
                if (Number(count) > possibleGame[color]) {
                    // determine if the count for that color is greater than what is possible
